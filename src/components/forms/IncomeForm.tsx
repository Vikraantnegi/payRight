...
export function IncomeForm() {
  ...
  const calculateCompletion = () => {
    ...
    const percentage = requiredComplete ? (optionalComplete / optionalFields.length) * 50 + 50 : (income.basicSalary > 0 ? 25 : 0);
    setCompletionPercentage(Math.round(percentage));
  };

  useEffect(() => {
    calculateCompletion();
    setIsValid(Object.keys(validationErrors).length === 0 && income.basicSalary > 0);
  ...
  const getIncomeBreakdown = () => {
    const breakdown = [];
    if (income.basicSalary > 0) breakdown.push({ label: 'Basic Salary', value: income.basicSalary, percentage: (income.basicSalary / getTotalIncome()) * 100 });
    if (income.hra > 0) breakdown.push({ label: 'HRA', value: income.hra, percentage: (income.hra / getTotalIncome()) * 100 });
    if (income.rsus > 0) breakdown.push({ label: 'RSUs', value: income.rsus, percentage: (income.rsus / getTotalIncome()) * 100 });
    if (income.performanceBonus > 0) breakdown.push({ label: 'Performance Bonus', value: income.performanceBonus, percentage: (income.performanceBonus / getTotalIncome()) * 100 });
    if (income.specialAllowance > 0) breakdown.push({ label: 'Special Allowance', value: income.specialAllowance, percentage: (income.specialAllowance / getTotalIncome()) * 100 });
    if (income.otherAllowances > 0) breakdown.push({ label: 'Other Allowances', value: income.otherAllowances, percentage: (income.otherAllowances / getTotalIncome()) * 100 });
    return breakdown;
  };

  return (
    <Card title="Income Details" subtitle="Enter your annual income from all sources" className="space-y-8">
      {/* Progress Indicator */}
      <div className="space-y-3">
        ...
        <div className="flex items-center space-x-2 text-xs text-gray-500">
          ...
          <span>{completionPercentage >= 100 ? 'Complete! Ready to proceed' : 'Fill in required fields to continue'}</span>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200/40">
        ...
        {getIncomeBreakdown().length > 0 && (
          <div className="space-y-3">
            <h4 className="text-sm font-medium text-gray-700 text-center mb-3">Income Breakdown</h4>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="text-sm text-amber-800">
            ...
            <ul className="space-y-1 text-xs">
              <li>• Basic salary is required and should be your core compensation</li>
              <li>• HRA is typically 40-50% of basic salary in metro cities</li>
              <li>• Include the annual value of RSUs/ESOPs for accurate calculations</li>
