...
export function HraCalculator() {
  ...
  const [calculationHistory, setCalculationHistory] = useState<
    { timestamp: Date; data: HraCalculation }[]
  >([]);

  const handleInputChange = (
    field: keyof HraCalculation,
    value: number | string
  ) => {
  ...
  const validateInputs = (data: HraCalculation) => {
    ...
    if (scenario === 'A') {
      ...
      if (data.actualRentPaid < 0) {
        ...
        newErrors.actualRentPaid =
          'Monthly rent seems unusually high. Please verify the amount.';
      }
    }
  ...
  const calculateHra = () => {
    ...
    const { basicSalary, hraComponent, cityType, monthlyRent, actualRentPaid } =
      calculation;
  ...
  useEffect(() => {
    ...
    calculation.hraComponent,
    calculation.cityType,
    calculation.monthlyRent,
    calculation.actualRentPaid,
  ]);

  const getCityMultiplier = () =>
    calculation.cityType === 'metro' ? 0.5 : 0.4;
  const getCityName = () =>
    calculation.cityType === 'metro' ? 'Metro Cities' : 'Non-Metro Cities';
  ...
  const getChartData = () => {
    ...
    return [
      {
        name: 'HRA Exemption',
        value: calculation.hraExemption,
        color: '#10b981',
      },
  ...
  const getBreakdownData = () => {
    ...
    return [
      ...
      {
        name: 'HRA Component',
        value: calculation.hraComponent,
        color: '#10b981',
      },
      {
        name: 'City Limit',
        value: calculation.basicSalary * getCityMultiplier(),
        color: '#8b5cf6',
      },
      {
        name: 'Rent - 10% Basic',
        value: Math.max(
          0,
          (calculation.actualRentPaid || 0) - calculation.basicSalary * 0.1
        ),
        color: '#f59e0b',
      },
    ];
  ...
  return (
    <Card
      ...
      subtitle='Optimize your HRA exemptions and maximize tax savings'
      className='space-y-8'
    >
      ...
      <div className='space-y-6'>
        <div className='text-center space-y-2'>
          <h3 className='text-xl font-semibold text-gray-800'>
            Choose Your Calculation Scenario
          </h3>
          <p className='text-gray-600'>
            Select the scenario that best fits your current situation
          </p>
        </div>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            ...
            <div className='text-center space-y-3'>
              ...
              <div className='space-y-2'>
                <div className='font-semibold text-lg'>
                  Scenario A: Rent Planning
                </div>
                <div className='text-sm opacity-80'>
                  I want to know how much rent I need to pay to fully utilize my
                  HRA
                </div>
              </div>
            </div>
          ...
          >
            <div className='text-center space-y-3'>
              ...
              <div className='space-y-2'>
                <div className='font-semibold text-lg'>
                  Scenario B: Exemption Calculation
                </div>
                <div className='text-sm opacity-80'>
                  I want to calculate my HRA exemption based on actual rent paid
                </div>
              </div>
            </div>
          </button>
        ...
        {showTips && (
          <div className='mt-4 space-y-3 text-sm text-amber-800'>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              ...
              <div>
                <div className='font-medium mb-2'>
                  📊 For Exemption Calculation (Scenario B):
                </div>
                <ul className='space-y-1 text-xs'>
          ...
          <div className='space-y-3'>
            ...
            <select
              ...
              onChange={e =>
                handleInputChange(
                  'cityType',
                  e.target.value as 'metro' | 'non-metro'
                )
              }
        ...
        <div className='flex items-start space-x-3 mb-4'>
          ...
          <div>
            ...
            <div className='text-sm text-blue-700'>
              HRA exemption is calculated as the <strong>minimum</strong> of
              these three amounts:
            </div>
          </div>
        </div>
          ...
          <div className='bg-white rounded-xl p-4 border border-blue-200'>
            <div className='text-center space-y-2'>
              <div className='text-2xl font-bold text-blue-600'>
                ...
                {Math.max(
                  0,
                  (calculation.actualRentPaid || 0) -
                    (calculation.basicSalary || 0) * 0.1
          ...
          <div className='bg-white rounded-xl p-4 border border-blue-200'>
            <div className='text-center space-y-2'>
              <div className='text-2xl font-bold text-blue-600'>
                ₹
                {(
                  (calculation.basicSalary || 0) * getCityMultiplier()
                ).toLocaleString('en-IN')}
              </div>
              <div className='text-xs text-blue-700 font-medium'>
                {getCityName()} Limit ({getCityMultiplier() * 100}%)
              </div>
            </div>
          </div>
        </div>
        ...
        <div className='space-y-8'>
          <div className='text-center space-y-4'>
            <h3 className='text-2xl font-bold text-gray-800'>
              Your HRA Analysis Results
            </h3>
            <p className='text-gray-600'>
              Comprehensive breakdown of your HRA benefits and tax savings
            </p>
          </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            ...
            <div className='bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200 rounded-2xl p-6 text-center transform hover:scale-105 transition-all duration-300'>
              ...
              <div className='text-xs text-orange-700 bg-orange-100 rounded-lg px-3 py-1'>
                Tax Impact: ₹
                {(calculation.taxableHra * 0.3).toLocaleString('en-IN')}
              </div>
            </div>
            ...
            {scenario === 'A' ? (
              ...
              <div className='bg-gradient-to-br from-purple-50 to-violet-50 border border-purple-200 rounded-2xl p-6 text-center transform hover:scale-105 transition-all duration-300'>
                ...
                <div className='text-3xl font-bold text-purple-600 mb-2'>
                  {calculation.hraComponent > 0
                    ? Math.round(
                        (calculation.hraExemption / calculation.hraComponent) *
                          100
                      )
                    : 0}
                  %
                ...
                {calculationHistory.map((item, index) => (
                  ...
                  >
                    ...
                    <div className='text-sm text-gray-700'>
                      ...
                      Exemption: ₹
                      {item.data.hraExemption.toLocaleString('en-IN')}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className='text-sm text-amber-700 space-y-3'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  <div>
                    <div className='font-medium mb-2'>
                      📋 Understanding HRA Limits:
                    </div>
                    <ul className='space-y-1 text-xs'>
                      <li>
                        • <strong>Actual HRA:</strong> What your company
                        provides
                      </li>
                      <li>
                        • <strong>Rent - 10% Basic:</strong> Rent paid minus 10%
                        of basic salary
                      </li>
                      <li>
                        • <strong>City Limit:</strong> 50% (metro) or 40%
                        (non-metro) of basic salary
                      </li>
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className='space-y-1 text-xs'>
                      <li>
                        • <strong>Rent Planning:</strong> Calculate optimal rent
                        for maximum benefits
                      </li>
                      <li>
                        • <strong>Documentation:</strong> Keep rent receipts and
                        rental agreement
                      </li>
                      <li>
                        • <strong>City Consideration:</strong> Metro cities
                        offer higher limits
                      </li>
                      <li>
                        • <strong>Tax Regime:</strong> HRA exemption only
