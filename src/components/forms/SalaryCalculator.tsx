...
export function SalaryCalculator() {
  ...
  const [calculationHistory, setCalculationHistory] = useState<
    { timestamp: Date; data: SalaryCalculation; breakdown: SalaryBreakdown }[]
  >([]);

  const handleInputChange = (
    field: keyof SalaryCalculation,
    value: number | string
  ) => {
  ...
  const validateInputs = (data: SalaryCalculation) => {
    ...
    if (data.basicSalary <= 0) {
      ...
      newErrors.basicSalary =
        'Basic salary seems unusually high. Please verify the amount.';
    }
    ...
    if (data.transportAllowance > 19200) {
      newErrors.transportAllowance =
        'Transport allowance above ₹19,200/year is taxable';
    }

    if (data.medicalAllowance > 15000) {
      newErrors.medicalAllowance =
        'Medical allowance above ₹15,000/year is taxable';
    }
  ...
  const calculateSalary = () => {
    ...
    if (taxRegime === 'old') {
      ...
      hraExemption = Math.min(
        ...
        Math.max(
          0,
          (city === 'metro' ? basicSalary * 0.5 : basicSalary * 0.4) -
            basicSalary * 0.1
        )
      );
  ...
  const getChartData = () => {
    ...
    return [
      ...
      {
        name: 'Allowances',
        value: breakdown.totalAllowances,
        color: '#10b981',
      },
      {
        name: 'Deductions',
  ...
  const getMonthlyComparisonData = () => {
    ...
    return [
      {
        month: 'Jan',
        gross: breakdown.grossSalary / 12,
        net: breakdown.monthlyInHand,
      },
      {
        month: 'Feb',
        gross: breakdown.grossSalary / 12,
  ...
  const getTaxRegimeComparison = () => {
    ...
    const oldRegimeTax = calculateTaxAmount(
      breakdown.taxableIncome +
        breakdown.standardDeduction +
        breakdown.hraExemption,
      'old'
    );
    const newRegimeTax = calculateTaxAmount(
      breakdown.taxableIncome + breakdown.standardDeduction,
      'new'
    );
    ...
    return {
      old: {
        tax: oldRegimeTax,
        netSalary:
          breakdown.grossSalary - breakdown.totalDeductions - oldRegimeTax,
        monthlyInHand:
          (breakdown.grossSalary - breakdown.totalDeductions - oldRegimeTax) /
          12,
      },
      new: {
        tax: newRegimeTax,
        netSalary:
          breakdown.grossSalary - breakdown.totalDeductions - newRegimeTax,
        monthlyInHand:
          (breakdown.grossSalary - breakdown.totalDeductions - newRegimeTax) /
          12,
      },
  ...
  return (
    <Card
      ...
      subtitle='Calculate your monthly take-home salary with detailed breakdowns and tax regime comparison'
      className='space-y-8'
    >
      ...
      <div className='space-y-6'>
        <div className='text-center space-y-2'>
          ...
          <p className='text-gray-600'>
            Choose the tax regime that applies to your situation
          </p>
        </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              <div>
                ...
                <ul className='space-y-1 text-xs'>
                  <li>
                    • <strong>Old Regime:</strong> Choose if you have
                    significant deductions
                  </li>
                  <li>
                    • <strong>New Regime:</strong> Choose for simplicity and
                    lower rates
                  </li>
                  <li>
                    • <strong>Compare both</strong> to find the best option
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className='space-y-1 text-xs'>
                  <li>
                    • <strong>Basic Salary:</strong> Affects EPF, gratuity, and
                    HRA
                  </li>
                  <li>
                    • <strong>Allowances:</strong> Some are tax-free up to
                    limits
                  </li>
                  <li>
                    • <strong>Deductions:</strong> Reduce taxable income
                  </li>
                </ul>
              </div>
            </div>
      ...
      <div className='space-y-3'>
        ...
        <select
          ...
          onChange={e =>
            handleInputChange('city', e.target.value as 'metro' | 'non-metro')
          }
        ...
        >
          <option value='metro'>
            🏙️ Metro Cities (Delhi, Mumbai, Chennai, Kolkata, Bangalore,
            Hyderabad)
          </option>
          <option value='non-metro'>
            🏘️ Non-Metro Cities (All other cities)
          </option>
        </select>
        <div className='flex items-center space-x-2 text-xs text-gray-500'>
          <InformationCircleIcon className='h-4 w-4' />
          <span>
            Affects HRA exemption limits and professional tax calculations
          </span>
        </div>
      </div>
      ...
      {calculation.basicSalary > 0 && (
        <div className='space-y-8'>
          <div className='text-center space-y-4'>
            ...
            <p className='text-gray-600'>
              Comprehensive analysis of your earnings and deductions
            </p>
          </div>

          {/* Tax Regime Info */}
          <div
            className={`p-6 rounded-2xl border ${getTaxRegimeBg(calculation.taxRegime)}`}
          >
            <div className='text-center space-y-3'>
              <div
                className={`text-xl font-semibold ${getTaxRegimeColor(calculation.taxRegime)}`}
              >
                {calculation.taxRegime === 'old'
                  ? 'Old Tax Regime'
                  : 'New Tax Regime'}{' '}
            ...
            <div className='bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-2xl p-6'>
              <div className='text-center space-y-4 mb-6'>
                ...
                <p className='text-indigo-600'>
                  See how your salary would differ under both tax regimes
                </p>
              </div>
                ...
                <div className='bg-white rounded-xl p-6 border border-green-200'>
                  <div className='text-center space-y-3'>
                    ...
                    <div className='text-2xl font-bold text-green-600'>
                      ₹
                      {getTaxRegimeComparison()?.old.monthlyInHand.toLocaleString(
                        'en-IN'
                      )}
                    </div>
                    <div className='text-sm text-green-600'>
                    ...
                    <div className='text-xs text-gray-500'>
                      Tax: ₹
                      {getTaxRegimeComparison()?.old.tax.toLocaleString(
                        'en-IN'
                      )}
                    </div>
                  </div>
                ...
                <div className='bg-white rounded-xl p-6 border border-blue-200'>
                  <div className='text-center space-y-3'>
                    ...
                    <div className='text-2xl font-bold text-blue-600'>
                      ₹
                      {getTaxRegimeComparison()?.new.monthlyInHand.toLocaleString(
                        'en-IN'
                      )}
                    </div>
                    ...
                    <div className='text-xs text-gray-500'>
                      Tax: ₹
                      {getTaxRegimeComparison()?.new.tax.toLocaleString(
                        'en-IN'
                      )}
                    </div>
                  </div>
              ...
              <div className='mt-6 text-center'>
                <div className='inline-flex items-center space-x-2 px-4 py-2 bg-white rounded-lg border border-indigo-200'>
                  ...
                  <span className='text-sm font-medium text-indigo-800'>
                    {(() => {
                      const comparison = getTaxRegimeComparison();
                      if (
                        !comparison?.old?.monthlyInHand ||
                        !comparison?.new?.monthlyInHand
                      ) {
                        return 'Tax regime comparison not available';
                      }
                      return comparison.old.monthlyInHand >
                        comparison.new.monthlyInHand
            ...
            <div className='space-y-3'>
              ...
              <div className='flex justify-between items-center py-3 border-b border-gray-100'>
                ...
                <span className='font-semibold text-blue-600'>
                  +₹{breakdown.totalAllowances?.toLocaleString('en-IN') || '0'}
                </span>
              </div>
              <div className='flex justify-between items-center py-3 border-b border-gray-100'>
                <span className='text-sm text-gray-700 font-medium'>
                ...
                <span className='font-semibold text-red-600'>
                  -₹{breakdown.totalDeductions?.toLocaleString('en-IN') || '0'}
                </span>
              </div>
              <div className='flex justify-between items-center py-3 border-b border-gray-100'>
                <span className='text-sm text-gray-700 font-medium'>
                ...
                <span className='font-semibold text-green-600'>
                  -₹
                  {breakdown.standardDeduction?.toLocaleString('en-IN') || '0'}
                </span>
              </div>
              {calculation.taxRegime === 'old' && (
                <div className='flex justify-between items-center py-3 border-b border-gray-100'>
                  ...
                  <span className='font-semibold text-green-600'>
                    -₹{breakdown.hraExemption?.toLocaleString('en-IN') || '0'}
                  </span>
                </div>
              )}
              <div className='flex justify-between items-center py-3 border-b border-gray-100'>
                ...
                <span className='font-semibold text-gray-900'>
                  ₹{breakdown.taxableIncome?.toLocaleString('en-IN') || '0'}
                </span>
              </div>
              <div className='flex justify-between items-center py-3 border-b border-gray-100'>
                <span className='text-sm text-gray-700 font-medium'>
                ...
                <span className='font-semibold text-red-600'>
                  -₹{breakdown.taxAmount?.toLocaleString('en-IN') || '0'}
                </span>
              </div>
                ...
                {calculationHistory.map((item, index) => (
                  ...
                  >
                    ...
                    <div className='text-sm text-gray-700'>
                      ...
                      Net: ₹
                      {item.breakdown.monthlyInHand.toLocaleString('en-IN')}
                      /month
                    </div>
                  </div>
                ))}
      ...
      <div className='bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-6'>
        <div className='flex items-start space-x-3'>
          ...
          <div className='space-y-4'>
            <div>
              <div className='font-semibold text-amber-800 text-lg mb-2'>
              ...
              <div className='text-sm text-amber-700 space-y-3'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  <div>
                    ...
                    <ul className='space-y-1 text-xs'>
                      <li>
                        • <strong>Basic Salary:</strong> Foundation for EPF,
                        gratuity, and HRA
                      </li>
                      <li>
                        • <strong>Allowances:</strong> Some are tax-free up to
                      ...
                      <li>
                        • <strong>Deductions:</strong> Reduce your taxable
                        income
                      </li>
                      <li>
                        • <strong>Tax Regime:</strong> Choose based on your
                        deduction profile
                      </li>
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className='space-y-1 text-xs'>
                      <li>
                        • <strong>EPF:</strong> Contribute maximum for tax
                        benefits
                      </li>
                      <li>
                        • <strong>HRA:</strong> Optimize rent to maximize
                        exemptions
                      </li>
                      <li>
                        • <strong>Allowances:</strong> Stay within tax-free
                        limits
                      </li>
                      <li>
                        • <strong>Comparison:</strong> Always compare both tax
                        regimes
                      </li>
                    </ul>
                  </div>
